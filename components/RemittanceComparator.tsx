...
export default function RemittanceComparator() {
  ...
  return (
    <section id="transfert" className="w-full max-w-3xl mx-auto px-4 py-10">
      <h2 className="text-2xl font-bold text-[#173a36] mb-1">{t('remittanceTitle')}</h2>
      ...
      {/* Amount input */}
      <div className="mb-6">
        <label className="block text-sm font-medium text-[#173a36]/70 mb-1">
          {t('remittanceAmount')}
        </label>
      ...
      {loading && !error && (
        <div className="space-y-3">
          {[0, 1, 2, 3, 4].map((i) => (
            <div key={i} className="h-16 rounded-xl bg-[#173a36]/5 animate-pulse" />
          ))}
        </div>
      )}
            ...
            <div
              ...
              className={`flex items-center gap-4 rounded-xl border px-4 py-3 ${
                i === 0
                  ...
                  : 'border-[#173a36]/10 bg-white'
              }`}
            >
              {/* Rank */}
              <span className="text-lg font-bold text-[#173a36]/40 w-6 shrink-0">
                {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `${i + 1}.`}
              </span>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-[#173a36]">{p.name}</span>
                  {i === 0 && (
                ...
                <div className="flex items-center gap-3 text-xs text-[#173a36]/50 mt-0.5">
              ...
              <div className="text-right shrink-0">
                <div className="font-bold text-[#173a36]">
                  {p.received.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} MAD
                </div>
                <div className="text-xs text-[#173a36]/40">{t('remittanceReceived')}</div>
              </div>
              ...
              <a
                ...
                className="shrink-0 flex items-center gap-1 rounded-lg bg-[#173a36] text-white text-sm font-medium px-3 py-2 hover:bg-[#173a36]/80 transition-colors"
              >
                {t('remittanceSend')}
