...
export default function BookingCards({ origin, destination, date }: { origin: string; destination: string; date?: string }) {
  ...
  useEffect(() => {
    ...
    (['ferry', 'flight'] as const).forEach(async (type) => {
      try {
        const response = await fetch(`/api/affiliates?type=${type}&origin=Europe&destination=Maroc`, { signal: controller.signal, cache: 'no-store' });
        if (!response.ok) return;
        const data = await response.json();
        const url = data.configured && verifiedPartnerUrl(data.affiliateUrl, type);
        if (url && !controller.signal.aborted) setPartners((current) => ({ ...current, [type]: url }));
      } catch {}
    });
