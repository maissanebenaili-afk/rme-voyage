    ...
    <section className="overflow-hidden rounded-3xl border border-[#0f1f3d]/10 bg-[#0f1f3d] shadow-xl">
      <div className="border-b border-white/10 bg-gradient-to-r from-[#0f1f3d] to-[#17345f] p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-wide text-emerald-200"><Radio size={12} className="animate-pulse" /> Direct officiel</div>
        </div>
      </div>
        ...
        <article className="rounded-2xl border border-white/10 bg-white/5 p-5">
          ...
          <div className="mt-5 grid gap-2 sm:grid-cols-3">
            {CHANNELS.map((channel) => (
              <a key={channel.name} href={channel.url} target="_blank" rel="noopener noreferrer" className="rounded-xl border border-white/10 bg-black/10 p-3 transition hover:border-[#f59e0b]/40">
