...
describe('Gate 3 — reproducible economic report', () => {
  ...
  test('7 retained, 5 eliminated, from the 12 real notices', async () => {
    const { packs } = await buildEvidencePacks(all());
    expect(packs.filter((p) => p.evaluation.retained).map((p) => p.facts.idweb)).toEqual(
      ['26-83998', '26-83332', '26-86504', '26-89746', '26-90272', '26-90281', '26-90875']);
    expect(packs.filter((p) => !p.evaluation.retained)).toHaveLength(5);
  });
