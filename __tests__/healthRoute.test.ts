...
describe('GET /api/health', () => {
  ...
  it('returns only aggregate provider state and no secret material', async () => {
    ...
    process.env = { ...previous, GROQ_API_KEY: 'SECRET_GROQ', OPENAI_API_KEY: 'SECRET_OPENAI', ANTHROPIC_API_KEY: 'SECRET_ANTHROPIC', GEMINI_API_KEY: 'SECRET_GEMINI', AI_ROUTER_FREE_ONLY: 'true' };
    try {
      const response = await GET();
      const body = await response.json();
